...
export default function NavBar() {
  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-white/80 dark:bg-black/80 border-b border-gray-200/50 dark:border-gray-800/50" role="banner">
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-center h-12">
        <nav className="hidden md:flex gap-8" aria-label="Main navigation">
          {links.map(l => (
            <Link key={l.href} href={l.href} className="nav-link" onClick={() => setOpen(false)}>{l.label}</Link>
          ))}
        </nav>
        <button aria-label={open ? 'Close menu' : 'Open menu'} aria-expanded={open} aria-controls="mobile-menu" className="md:hidden text-gray-900 dark:text-gray-100 absolute right-6" onClick={() => setOpen(o => !o)}>
          {open ? '✕' : '☰'}
        </button>
      </div>
      {open && (
