  ...
  return (
    <div className="prose dark:prose-invert max-w-none print:pt-0">
      <header className="mb-8">
        <h1 className="m-0">{t('profile.name')}</h1>
        <p className="m-0 text-sm">{t('profile.title')} | {t('profile.location')} | {cv.email}</p>
      ...
      <button onClick={() => window.print()} className="btn-primary print:hidden">{t('cv.print')}</button>
    </div>
  );
}
